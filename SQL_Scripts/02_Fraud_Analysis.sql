...
-- 1. IDENTIFYING SUSPICIOUS ACCOUNTS (SMURFING)
-- Flagging accounts with more than 5 transactions in a single day
WITH fraud AS (
    SELECT account_id, date, count(trans_id) AS total_trans_same_day, SUM(amount) AS fraud_amount
    FROM trans
    GROUP BY account_id, date
    HAVING count(trans_id) > 5
    ORDER BY total_trans_same_day DESC
)
...
-- 2. DEMOGRAPHIC PROFILING: GENDER DISTRIBUTION
...
-- 3. DEMOGRAPHIC PROFILING: AVERAGE AGE AT TIME OF FRAUD
...
-- 4. GEOSPATIAL ANALYSIS: FRAUD HOTSPOTS
-- Identifying the districts with the highest concentration of fraud suspects
WITH fraud AS (
    SELECT t.account_id, c.client_id, c.gender, TIMESTAMPDIFF(YEAR, c.Birth_Date, t.date) AS age, ds.A2
    FROM trans t
    JOIN disp d ON t.account_id = d.account_id
    JOIN client c ON d.client_id = c.client_id
    JOIN district ds ON c.district_id = ds.A1
    WHERE d.type = 'OWNER'
    GROUP BY t.account_id, t.date, c.client_id, c.gender, c.Birth_Date, ds.A2
    HAVING count(t.trans_id) > 5
)
...
-- 5. BUST-OUT FRAUD INVESTIGATION (THE LOAN HEIST)
-- Cross-referencing fraud suspects with loan defaults (Status D: default/unpaid)
WITH fraud AS (
    SELECT t.account_id, c.client_id, l.status, l.amount
    FROM trans t
    JOIN disp d ON t.account_id = d.account_id
    JOIN client c ON d.client_id = c.client_id
    ...
    WHERE d.type = 'OWNER'
      AND l.status IN ('A', 'D')
    GROUP BY t.account_id, c.client_id, l.status, l.amount
    HAVING count(t.trans_id) > 5
)
